...
def html만들기(A, 거래들, 입력파일, 자산, 보험, 가족들, 종목, 조언, 뉴스, 재무,
    ...
    # 예시 자료로 만든 화면은 "실제 거래가 들어 있다"고 적으면 안 된다.
    # 이 파일을 그대로 남에게 보여줄 수도 있기 때문이다.
    자료안내 = (
        '<span class="warn">이 화면의 가족·금액·거래는 전부 지어낸 교육용 예시입니다.</span> '
        '실제 인물이나 가구의 자료가 아닙니다.'
        if 예시자료 else
...
def main():
    ...
    출력.write_text(html만들기(A, 거래들, 입력.name, 자산, 보험, 가족들, 종목, 조언, 뉴스, 재무,
                        예시자료="sample" in 자료.as_posix()), encoding="utf-8")
    앱으로만들기(출력.parent)
