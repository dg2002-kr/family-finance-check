...
def 규칙키워드(제목: str) -> str:
    ...
    키 = " ".join(낱말[-2:]) if len(낱말) >= 2 else (낱말[0] if 낱말 else 제목[:8])
    return 키[:14]


def AI키워드(키, 모델, 제목들):
    """제목을 2~6글자 키워드로 줄인다. 실패하면 규칙으로 돌아간다."""
    ...
    지시 = ("뉴스 제목을 한눈에 알아볼 짧은 키워드로 줄입니다.\n"
           "- 2~7글자. 명사 위주로 씁니다.\n"
           "- 제목에 있는 말만 씁니다. 없는 내용을 지어내지 않습니다.\n"
           "- 좋고 나쁨을 판단하거나 전망을 덧붙이지 않습니다.\n"
           "- 예: '추석 앞두고 삼성전자 냉장고 잇단 먹통' → '냉장고 먹통'")
    ...
    try:
        ...
        표 = {x["번호"]: x["키워드"].strip()[:14] for x in 결과.get("키워드", [])}
