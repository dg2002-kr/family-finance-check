...
# ── 누구를 위해 쓴 돈인가 ────────────────────────────────────────────────
# 돈을 낸 사람과 그 돈이 쓰인 사람은 다릅니다.
# 아이 학원비가 엄마 카드에서 나가도, 그건 아이를 위해 쓴 돈입니다.
#
# 아래 카테고리는 특정 개인이 아니라 온 가족이 함께 쓴 것으로 봅니다.
가족공통카테고리 = {"주거", "장보기", "통신비", "생활", "여가"}

# 가맹점 이름으로 수혜자를 정하는 규칙은 입력 폴더의 `수혜자규칙.csv` 에 적습니다.
# (컬럼: 키워드, 수혜자 / 수혜자에 `가족공통` 이라고 적어도 됩니다)
수혜자규칙파일 = "수혜자규칙.csv"
...
def 수혜자규칙읽기(루트: Path):
    """입력 폴더의 수혜자규칙.csv 를 읽는다. 없으면 빈 목록."""
    path = 루트 / 수혜자규칙파일
    if not path.exists():
        return []
    text, _, _ = 인코딩찾기(path)
    ...
    for r in csv.DictReader(text.splitlines()):
        ...
        사람 = 정규화(r.get("수혜자"))
        if 키 and 사람:
            규칙.append((키.lower(), 사람))
    규칙.sort(key=lambda kv: -len(kv[0]))     # 긴 키워드를 먼저 본다
    return 규칙


def 수혜자정하기(가맹점: str, 카테고리: str, 결제자: str, 규칙) -> str:
    이름 = 가맹점.lower()
    for 키, 사람 in 규칙:
        if 키 in 이름:
            return 사람
...
# ----------------------------------------------------------------- 파일 1개 처리
def 파일읽기(path: Path, 사람: str, 루트: Path, 수혜자규칙=()):
    ...
    for n, row in enumerate(rows[h + 1:], start=h + 2):
        ...
        카테고리 = ("이체" if 구분 == "이체" else
                 "수입" if 구분 == "수입" else
                 카테고리정하기(가맹점, 셀("원본분류")))
        결과.append({
            "날짜": 날짜,
            "사람": 사람,
            "수혜자": 수혜자정하기(가맹점, 카테고리, 사람, 수혜자규칙),
            "원본파일": f"{상대경로}:{n}행",
            "구분": 구분,
            "가맹점": 가맹점,
            "금액": 금액,
...
def main():
    ...
    수혜자규칙 = 수혜자규칙읽기(루트)
    if 수혜자규칙:
        print(f"수혜자 규칙 {len(수혜자규칙)}개를 읽었습니다 ({수혜자규칙파일})")
    print()
    ...
    for 사람폴더 in sorted(p for p in 루트.iterdir() if p.is_dir()):
        ...
        for f in 파일들:
            try:
                rows, (enc, n, skip) = 파일읽기(f, 사람, 루트, 수혜자규칙)
            except 읽기실패 as e:
                실패.append(str(e))
                continue
            거래들.extend(rows)
    ...
    수혜자별 = {}
    for t in 거래들:
        수혜자별.setdefault(t["수혜자"], []).append(t)

    print()
    print(f"읽은 파일 {읽은파일}개 / 거래 {len(거래들)}건 / 기간 {달들[0]} ~ {달들[-1]}")
    print("누가 결제했나: " + " · ".join(
        f"{이름} {지출합(ts):,}원" for 이름, ts in sorted(사람별.items(), key=lambda kv: -지출합(kv[1]))))
    print("누구를 위해 썼나: " + " · ".join(
        f"{이름} {지출합(ts):,}원" for 이름, ts in sorted(수혜자별.items(), key=lambda kv: -지출합(kv[1]))))
